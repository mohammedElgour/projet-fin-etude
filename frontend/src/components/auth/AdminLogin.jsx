...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Login successful.', `Welcome back, ${data?.user?.name || 'Admin'}.`);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-50 via-white to-primary-50 px-4 py-12 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 sm:px-6 lg:px-8">
      ...
      >
        ...
        <h2 className="mb-6 text-center text-3xl font-bold text-slate-900 dark:text-white">Admin</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            ...
            <span className="ml-2 inline-block h-5 w-5 rounded-full bg-white/20 transition-all duration-300 group-hover:h-6 group-hover:w-6" />
          </motion.button>
        </form>
      </motion.div>
    </div>
